...
//	$scope.changemonthforbudgetData(sessionStorage.ReportingMonth,sessionStorage.ReportingYear);
    ...
    $http({
  		...
  		// toaster.pop('warning', "data retrived");
  		////alert(response.status);
  		if(response.status == 200 && response.status != 204){
		...
		// //alert("flag_status== "+obj[0].flag_status);
		 if(obj[0].flag_status == 1){
			 ...
			 $scope.submitDisabledbutton="";
			 
		 }else{
			 
			 $scope.disablefields_1="true"; 
			 $scope.submitDisabledbutton="true";
		 }
 ...
 //$scope.form_ReportingYear = sessionStorage.ReportingYear
	...
	////alert("$scope.ReporingDate "+$scope.ReporingDate);
	 ...
	 $http({
    	...
    	//alert("status  00 "+response.status)
...
//State data save and submit
	$scope.submitStateClick = function(event) {
		
	// //alert("this is data inside save"+e);	
		//console.log(myJSON);
	    ...
	    $scope.District_id1=$scope.District_id;
	     ...
	     $scope.release_date = $scope.release_date;
       ...
       if($scope.AllocatedBudget_state < ($scope.Amount_tag ||$scope.Amount_Q1 || $scope.Amount_Q2 || $scope.Amount_Q3)){
	    	 
	    	 //alert("if value is more "+$scope.AllocatedBudget_state+" dhc_buget "+$scope.Amount_Q4); 
	    	 //event.preventDefault();
	    	 event.stopPropagation();
	    	 
	     }else{
	    	 
	    	 //alert("buget_stat1  "+$scope.AllocatedBudget_state+" dhc_buget1 "+$scope.Amount_Q4); 
	   ...
	   ////alert("state_budget_master_id   "+sessionStorage.state_budget_master_id+"  sessionStorage.quarter_no "+sessionStorage.quarter_no);
	     ...
	     if(sessionStorage.quarter_no== "Q0"){
	    	...
	    	// ////alert(sessionStorage.state_budget_master_id);
	 	    ...
	 	    $scope.Qdata="Q1 and master data of "+$scope.District_id2[0];
	 	   $scope.Amount_tag = $scope.Amount;
           ...
           //  var b = 1;
        	...
        	////alert(Indata1);
	    	 ...
	    	 $scope.Qdata="Q2";
	    	 $scope.Amount_tag = $scope.Amount_Q2;
	    	 $scope.release_date_Q2 = $scope.release_date_Q2;
	    	 ...
	    	 $scope.Qdata="Q3";
	    	 $scope.Amount_tag = $scope.Amount_Q3;
	    	 $scope.release_date_Q3 = $scope.release_date_Q3;
	    	 ...
	    	 $scope.Qdata="Q4";
	    	 $scope.Amount_tag=$scope.Amount_Q4;
	    	 $scope.release_date_Q4 = $scope.release_date_Q4;
